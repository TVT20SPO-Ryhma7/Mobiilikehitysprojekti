package com.example.mobiilikehitysprojekti.tetris_view
...
import com.example.mobiilikehitysprojekti.TetrisGame
...
import com.example.mobiilikehitysprojekti.tetris_models.Block
...
class TetrisView : View {
    ...
    private var activity: TetrisGame? = null
    private val viewHandler = ViewHandler(this)
    private var cellSize: Dimension = Dimension(0, 0)
    private var frameOffset: Dimension = Dimension(0, 0)

    //This code is made to generate the area where the game takes place
    ...
    private class ViewHandler (private val owner: TetrisView) : Handler(Looper.getMainLooper()){
        //ViewHandler is a custom Handler that caters for the games message-sending and processing needs

        override fun handleMessage(message: Message) {
            if (message.what == 0) {  //When TetrisView possesses a model that is not 0, the following code line will check certain statuses
                if (owner.model != null) {
                    if (owner.model!!.isGameOver()) {
                        owner.model?.endGame()
                        Toast.makeText(owner.activity, "Game over", Toast.LENGTH_LONG).show()
                    }//If the game is over, it will show a pop-up that says "Game Over"
        ...
        } //sleep() simply removes any previous message and send a new one with a delay
    }

    private data class Dimension(val width: Int, val height: Int)
            //Dimension simply holds two properties: width and height. Dimension needed to be added for
            //some of the private values at the start

    fun setModel(model: AppModel) {
        this.model = model
    } //Sets the current model in the view

    fun setActivity(gameActivity: TetrisGame) {
        this.activity = gameActivity
    }//Sets the activity in use
    ...
    fun setGameCommand(move: AppModel.Motions) {
        if (null != model && (model?.currentState == AppModel.Statuses.ACTIVE.name)) {
            if (AppModel.Motions.DOWN == move) {
                ...
                invalidate() //Requests a change on screen via call to onDraw()
                return
            }
            setGameCommandWithDelay(move)
        }//Sets the current motion of the model
    }
